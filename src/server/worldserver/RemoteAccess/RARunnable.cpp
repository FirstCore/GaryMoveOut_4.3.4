...
void RARunnable::run()
{
    ...
    {
        sLog->outError(LOG_FILTER_WORLDSERVER, "Arkcore RA can not bind to port %d on %s", raport, stringip.c_str());
        return;
    }

    sLog->outInfo(LOG_FILTER_WORLDSERVER, "Starting Arkcore RA on port %d on %s", raport, stringip.c_str());
    ...
    sLog->outDebug(LOG_FILTER_WORLDSERVER, "Arkcore RA thread exiting");
}
