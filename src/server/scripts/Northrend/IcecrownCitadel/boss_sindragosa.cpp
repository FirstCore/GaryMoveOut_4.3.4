        ...
        class spell_sindragosa_unchained_magic_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                ...
                if (unitList.size() > maxSize)
                    Arkcore::Containers::RandomResizeList(unitList, maxSize);
            }
...
class spell_frostwarden_handler_order_whelp : public SpellScriptLoader
{
    public:
        ...
        class spell_frostwarden_handler_order_whelp_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                targets.remove_if(Arkcore::ObjectTypeIdCheck(TYPEID_PLAYER, false));
                if (targets.empty())
                    return;

                WorldObject* target = Arkcore::Containers::SelectRandomContainerElement(targets);
                targets.clear();
            ...
            void HandleForcedCast(SpellEffIndex effIndex)
            {
                ...
                Arkcore::Containers::SelectRandomContainerElement(unitList)->CastSpell(GetHitUnit(), uint32(GetEffectValue()), true);
            }
