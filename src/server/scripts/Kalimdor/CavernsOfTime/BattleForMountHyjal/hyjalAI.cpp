...
{
    CellCoord pair(Arkcore::ComputeCellCoord(x, y));
    Cell cell(pair);
    cell.SetNoCreate();

    // First get all creatures.
    std::list<Creature*> creatures;
    Arkcore::AllFriendlyCreaturesInGrid creature_check(me);
    Arkcore::CreatureListSearcher<Arkcore::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <Arkcore::CreatureListSearcher<Arkcore::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    Arkcore::WorldObjectWorker<Arkcore::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        CellCoord pair(Arkcore::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
        Cell cell(pair);
        cell.SetNoCreate();
        ...
        TypeContainerVisitor
            <Arkcore::CreatureListSearcher<Arkcore::AllFriendlyCreaturesInGrid>,
