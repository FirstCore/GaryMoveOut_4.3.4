...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 Sender, uint32 action)
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                player->TeleportTo(657, -344.37f, 24.96f, 626.98f, 4.20f);
            break;
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (instance && instance->GetData(DATA_GRAND_VIZIER_ERTAN)==DONE)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Retour Sommet du Cyclone", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
		
		if (instance && instance->GetData(DATA_GRAND_VIZIER_ERTAN)==DONE)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Aller a la Cime du Nimbus", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
		
		if (instance && instance->GetData(DATA_ALTAIRUS)==DONE)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Aller au Temple d'Asaad", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
