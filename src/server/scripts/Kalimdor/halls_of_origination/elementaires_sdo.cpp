...
#define spell_prisonnier_de_la_bulle 91158 // Prisonnier de la bulle

class mob_gardien_deau: public CreatureScript
{
public:
       mob_gardien_deau() : CreatureScript("mob_gardien_deau") {}
 
       struct mob_gardien_deauAI : public ScriptedAI
       {
            mob_gardien_deauAI(Creature * pCreature) : ScriptedAI(pCreature) {}
			
			uint32 prisonnier_de_la_bulle;
			
		    void Reset()
			{
			 prisonnier_de_la_bulle = 5000;
			}
            ...
            if (prisonnier_de_la_bulle<= diff)
			{
			if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 0, true))
			DoCast(pTarget, spell_cisaille_de_vent);
			prisonnier_de_la_bulle = 15000;
			} else prisonnier_de_la_bulle -= diff;
	...
	CreatureAI* GetAI(Creature* pCreature) const
    {
         return new mob_gardien_deauAI(pCreature);
    }
