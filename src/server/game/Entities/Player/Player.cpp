...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
    ...
    {
        case FATIGUE_TIMER:
            if (sWorld->getBoolConfig(CONFIG_DISABLE_FATIGUE))
        ...
        case FIRE_TIMER:
        {
            if (!isAlive() || sWorld->getBoolConfig(CONFIG_DISABLE_FATIGUE))
                return DISABLED_MIRROR_TIMER;
...
bool Player::takeCredit(int32 prix)
{
    // Récupération du nombre de crédits
    QueryResult result = LoginDatabase.PQuery("SELECT credits FROM account_site WHERE id_game=%u LIMIT 1;", GetSession()->GetAccountId());
    if (!result)
    {
        ChatHandler(GetSession()).PSendSysMessage("Erreur : Nous ne trouvons pas vos crédits ! Merci de contacter un MJ."); 
        return false;
    }
    uint32 credits = (*result)[0].GetUInt32();

   // Paiement
    if (credits < prix)
    {
        ChatHandler(GetSession()).PSendSysMessage("Erreur : Vous n'avez pas assez de crédits.");
        return false;
    }

    LoginDatabase.PQuery("UPDATE account_site SET credits = %u WHERE id_game = %u;", (credits - prix), GetSession()->GetAccountId());
    return true;
 }
...
{
    if (cr == CR_CRIT_TAKEN_MELEE || cr == CR_CRIT_TAKEN_RANGED || cr == CR_CRIT_TAKEN_SPELL)
        return float((1.0f - pow(0.99f, GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + cr) / 79.12785f)) * 50);
    ...
    if (crItem->ExtendedCost) // case for new honor system
    {
        ...
        for (int i = 0; i < MAX_ITEM_EXT_COST_CURRENCIES; ++i)
        {
            if (iece->RequiredCurrency[i])
            {
                ...
                uint32 precision = (entry->Flags & CURRENCY_FLAG_HIGH_PRECISION) ? 100 : 1;
                ModifyCurrency(iece->RequiredCurrency[i], -int32(iece->RequiredCurrencyCount[i] / precision), true, true);
            }
        }
    }
