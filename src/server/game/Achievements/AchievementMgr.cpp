...
void AchievementMgr<T>::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    {
        Arkcore::AchievementChatBuilder say_builder(*GetOwner(), CHAT_MSG_GUILD_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED, achievement->ID);
    ...
    {
        CellCoord p = Arkcore::ComputeCellCoord(GetOwner()->GetPositionX(), GetOwner()->GetPositionY());

        Cell cell(p);
        cell.SetNoCreate();

        Arkcore::AchievementChatBuilder say_builder(*GetOwner(), CHAT_MSG_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED, achievement->ID);
        Arkcore::LocalizedPacketDo<Arkcore::AchievementChatBuilder> say_do(say_builder);
        Arkcore::PlayerDistWorker<Arkcore::LocalizedPacketDo<Arkcore::AchievementChatBuilder> > say_worker(GetOwner(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), say_do);
