...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Arkcore::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Arkcore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Arkcore::ComputeCellCoord(x + radius, y + radius).normalize();
