...
bool BattlefieldTB::SetupBattlefield()
{
    ...
    for (uint8 i=0; i<BATTLEFIELD_TB_GY_MAX; i++)
    {
        ...
        {
            // In no war time Gy is control by defender712
...
/* TODO PATILUS ==> A debug avec la fonction OnGameObjectCreate
...
ShowNpc(creature, true);
    ...
    if (!endbytimer)
        SendWarningToAllInZone((GetDefenderTeam() == TEAM_ALLIANCE) ? BATTLEFIELD_TB_TEXT_WIN_KEEP : BATTLEFIELD_TB_TEXT_WIN_KEEP + 1);
    else
        SendWarningToAllInZone((GetDefenderTeam() == TEAM_ALLIANCE) ? BATTLEFIELD_TB_TEXT_DEFEND_KEEP : BATTLEFIELD_TB_TEXT_DEFEND_KEEP + 1);
}
...
BfCapturePointTB::BfCapturePointTB(BattlefieldTB* battlefield, TeamId teamInControl) : BfCapturePoint(battlefield)
{
    m_Bf = battlefield;
