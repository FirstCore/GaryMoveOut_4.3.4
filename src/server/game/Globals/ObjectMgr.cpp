...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveYard(float x, float y, float z, uint32 MapId, uint32 team)
{
    ...
    AreaTableEntry const* areaEntry = GetAreaEntryByAreaID(zoneId);
    uint32 count = 0;
    while (areaEntry->zone != 0)
    {
        if (count > 4)
        {
            sLog->outError(LOG_FILTER_GENERAL, "Sub-ZoneId not found for map %u coords (%f, %f, %f)", MapId, x, y, z);
            return GetDefaultGraveYard(team);
        }
        count += 1;
        zoneId = areaEntry->zone;
        areaEntry = GetAreaEntryByAreaID(zoneId);
    }
...
void ObjectMgr::LoadReputationOnKill()
{
    ...
    //   10           11           12           13              14              15
        "CurrencyId1, CurrencyId2, CurrencyId3, CurrencyCount1, CurrencyCount2, CurrencyCount3 FROM creature_onkill_reputation");
    ...
    do
    {
        ...
        if (repOnKill.currencyid1)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", repOnKill.currencyid1);
                continue;
            }
        }
        ...
        if (repOnKill.currencyid2)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", repOnKill.currencyid2);
                continue;
            }
        }
        ...
        if (repOnKill.currencyid3)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", repOnKill.currencyid3);
                continue;
            }
        }
